...
<p class="eyebrow reveal">Evaluasi Akhir</p><h2 class="reveal">Asesmen Komprehensif</h2><p class="lede reveal">Jawab seluruh soal setelah menuntaskan buku. Skor akan ditampilkan setelah semua jawaban diperiksa.</p><div class="card reveal">${items.map(([q, c, a, b]) => `<div class="assess-q" data-correct="1"><p>${q}</p><button class="quiz-opt" onclick="selectAssessAnswer(this)">${c}</button><button class="quiz-opt" onclick="selectAssessAnswer(this)">${a}</button><button class="quiz-opt" onclick="selectAssessAnswer(this)">${b}</button></div>`).join('')}<button class="done-btn" onclick="checkAssessment()">Periksa Jawaban Saya</button><div class="assess-result" id="asesmenResult"></div></div><div class="chapter-nav reveal"><button class="nav-btn prev" onclick="goPrev()">â† Sebelumnya</button><button class="nav-btn next" onclick="goNext()">Selanjutnya â†’</button></div>`;
  const chapters = [
    ['Memahami Komplain di Lingkungan Sekolah Islam','Komplain bukan gangguan yang harus dibungkam, melainkan sinyal bahwa ada jarak antara harapan pemangku kepentingan dan pengalaman yang mereka rasakan di sekolah.',[
      ...
      ['Menurut bab ini, komplain pada dasarnya adalahâ€¦','sinyal adanya kesenjangan antara harapan dan pengalaman layanan','serangan yang harus segera dibantah','bukti bahwa sekolah selalu gagal'],
      ['Orang tua mengeluhkan perubahan jadwal melalui grup. Respons awal yang paling tepat adalahâ€¦','menerima pesan dengan tenang, mengumpulkan fakta, lalu menjelaskan tindak lanjut','menghapus pesannya agar diskusi berhenti','menyuruh orang tua mencari informasi sendiri'],
    ...
    ['Landasan Komunikasi Humanis dalam Pelayanan Sekolah Islam','Komunikasi yang baik tidak berhenti pada kata-kata sopan. Ia menuntut kehadiran, kejelasan, empati, dan adab agar pihak yang mengeluh tetap merasa dihargai sekalipun masalahnya belum dapat selesai seketika.',[
      ['A. Empati sebagai Titik Berangkat','Empati berarti berusaha memahami perasaan dan kebutuhan pihak lain tanpa buru-buru menyetujui semua tuntutannya. Kalimat seperti â€œKami memahami kekhawatiran Bapak/Ibuâ€ membuka ruang dialog, karena lawan bicara merasa didengar sebelum sekolah menjelaskan kebijakan atau fakta yang ada.'],
      ...
      ['Mendengar aktif dilakukan denganâ€¦','membiarkan pihak lain menjelaskan, merangkum inti, lalu mengklarifikasi fakta','langsung menawarkan solusi sebelum memahami masalah','mengalihkan pembicaraan ke kesalahan pihak pengadu'],
      ...
      ['Staf administrasi menerima pesan yang emosional pada malam hari. Respons profesional pertama adalahâ€¦','mengakui penerimaan pesan dan menetapkan waktu tindak lanjut yang jelas','membalas dengan nada yang sama','meneruskan tangkapan layar ke grup umum']
    ]],
    ['Strategi Menangani Komplain secara Efektif dan Humanis','Penanganan komplain yang elegan memiliki alur: menerima dengan tenang, memahami secara utuh, menentukan tindak lanjut, lalu memastikan perbaikan benar-benar terasa.',[
      ...
      ['Langkah pertama saat menerima komplain adalahâ€¦','menerima dengan tenang dan mencatat inti persoalan','menentukan siapa yang salah di depan pengadu','menjelaskan aturan tanpa mendengar'],
    ...
    ['Mengubah Komplain Menjadi Kepercayaan','Kepercayaan tidak pulih hanya karena sekolah berkata maaf. Ia tumbuh ketika sekolah bertanggung jawab, terbuka terhadap pembelajaran, dan menunjukkan perubahan yang dapat dirasakan.',[
      ['A. Minta Maaf secara Bertanggung Jawab','Permintaan maaf yang baik menyebut dampak yang diakui, bukan sekadar berkata â€œmaaf jika tersinggungâ€. Bila sekolah memang keliru, akui bagian tersebut tanpa mencari kambing hitam; bila fakta belum lengkap, sampaikan bahwa pemeriksaan sedang dilakukan dengan sungguh-sungguh.'],
      ...
      ['Permintaan maaf yang bertanggung jawab perluâ€¦','mengakui dampak dan diikuti langkah perbaikan','dibuat panjang agar terdengar meyakinkan','disertai pembelaan terhadap semua kesalahan'],
      ['Komplain berulang tentang informasi pembayaran paling tepat dibaca sebagaiâ€¦','data untuk memperbaiki alur komunikasi dan layanan','alasan untuk menghentikan pertanyaan orang tua','kesalahan pribadi setiap pengadu'],
      ['Setelah kesalahan jadwal terjadi, sekolah sudah meminta maaf. Agar kepercayaan pulih, sekolah perluâ€¦','memberi pembaruan berkala dan menunjukkan perbaikan jadwal yang nyata','menganggap masalah otomatis selesai','meminta orang tua tidak membahasnya lagi']
    ]],
    ['Membangun Pelayanan Prima di Sekolah Islam','Pelayanan prima adalah pengalaman yang konsisten: warga sekolah mudah mendapat informasi, diperlakukan hormat, dan tahu ke mana harus meminta bantuan ketika menghadapi kendala.',[
      ...
      ['Tujuan standar layanan adalahâ€¦','menjaga mutu respons tetap konsisten','membatasi staf agar tidak boleh membantu','membuat semua masalah menjadi urusan pimpinan'],
      ...
      ['Dua staf memberi jawaban berbeda tentang prosedur izin. Perbaikan paling tepat adalahâ€¦','menyepakati standar dan sumber informasi bersama','membiarkan orang tua memilih jawaban yang disukai','menyalahkan salah satu staf di hadapan orang tua']
    ]],
    ['Membangun Budaya Sekolah yang Humanis dan Responsif','Komplain akan ditangani lebih baik bila seluruh sekolah memiliki budaya yang tidak alergi terhadap masukan, berani bertanggung jawab, dan saling menguatkan dalam memperbaiki layanan.',[
      ...
      ['Budaya responsif dimulai ketika pimpinanâ€¦','menerima kabar sulit tanpa mempermalukan pembawa pesan','menutup semua jalur masukan','hanya mendengar staf yang setuju dengannya'],
      ...
      ['Rapat evaluasi komplain yang sehat sebaiknyaâ€¦','mencari pola perbaikan sistem, bukan kambing hitam','membahas kesalahan orang secara terbuka','menunda semua tindak lanjut sampai akhir tahun']
    ]]
  ];
  const items = [
    ['Komplain menunjukkanâ€¦','ada kesenjangan antara harapan dan pengalaman layanan','sekolah tidak perlu mendengar masukan','semua pengadu berniat buruk'],
    ['Pihak yang dapat menyampaikan komplain adalahâ€¦','orang tua, siswa, guru, staf, atau masyarakat','hanya kepala sekolah','hanya orang tua'],
    ['Sikap awal yang tepat saat menerima komplain ialahâ€¦','tenang dan mendengarkan','membantah secepatnya','menghindari pengadu'],
    ['Mendengar aktif mencakupâ€¦','merangkum dan mengklarifikasi inti masalah','menyela untuk memberi aturan','mengalihkan topik'],
    ['Empati berartiâ€¦','memahami pengalaman pihak lain tanpa tergesa menghakimi','menyetujui semua tuntutan','mengabaikan emosi'],
    ['Klarifikasi diperlukan untukâ€¦','membawa percakapan dari asumsi ke fakta','menambah ketegangan','menutup dialog'],
    ['Janji tindak lanjut harusâ€¦','realistis serta memiliki tenggat','dibuat sebanyak mungkin','disampaikan tanpa catatan'],
    ['Dokumentasi komplain membantuâ€¦','memantau solusi dan pembelajaran sistem','menyebarkan masalah','menambah hukuman'],
    ['Permintaan maaf yang baik diikutiâ€¦','tindakan perbaikan','pembelaan panjang','pengalihan isu'],
    ['Transparansi berartiâ€¦','memberi penjelasan proses sambil menjaga privasi','membuka seluruh data pribadi','tidak memberi kabar sama sekali'],
    ['Komplain berulang perlu dipakai sebagaiâ€¦','data perbaikan layanan','alasan menutup kanal masukan','bukti pengadu tidak loyal'],
    ['Standar layanan menjagaâ€¦','konsistensi mutu respons','staf tidak dapat membantu','masalah tidak dilaporkan'],
    ['Pemetaan perjalanan pengguna membantu sekolahâ€¦','menemukan titik layanan yang menyulitkan','memperpanjang proses','fokus pada satu unit saja'],
    ['Pelatihan tim diperlukan agarâ€¦','anggota memahami prosedur dan komunikasi','setiap orang bekerja sendiri','jawaban selalu berbeda'],
    ['Pimpinan yang humanis akanâ€¦','membuka ruang pelaporan masalah','mempermalukan pembawa kabar','menolak kritik'],
    ['Koordinasi lintas peran mencegahâ€¦','pengadu mengulang cerita dan kasus terabaikan','adanya solusi bersama','catatan tindak lanjut'],
    ['Evaluasi yang sehat berfokus padaâ€¦','pola perbaikan','kambing hitam','menyimpan masalah'],
    ['Jika fakta belum lengkap, sekolah sebaiknyaâ€¦','menyampaikan pemeriksaan sedang dilakukan','membuat janji palsu','menuduh pengadu'],
    ['Keragaman sumber komplain perlu dipandang sebagaiâ€¦','beragam sudut pandang yang harus diperiksa adil','alasan membedakan hak layanan','beban satu unit saja'],
    ['Hasil akhir penanganan komplain yang elegan adalahâ€¦','kepercayaan dan layanan yang lebih baik','percakapan yang cepat berhenti','kemenangan salah satu pihak']
  ];
